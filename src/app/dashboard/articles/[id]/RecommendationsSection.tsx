...
export function RecommendationsSection({ articleId }: RecommendationsSectionProps) {
  ...
  const fetchRecs = useCallback(
    async (cursorParam: string | null) => {
      ...
      setError(null);
      try {
        const params = new URLSearchParams({ articleId, limit: String(PAGE_SIZE) });
    ...
    [articleId, activeStatus]
  );

  // Re-fetch when status tab changes; reset pagination
  useEffect(() => {
    setCursor(null);
  ...
  const handleSingle = useCallback(
    async (id: string, newStatus: "accepted" | "dismissed") => {
      ...
      // Optimistic update
      setRecs((prev) =>
        prev.map((r) => (r.id === id ? { ...r, status: newStatus as RecStatus } : r))
      );
      ...
      try {
        ...
        // Sync server state (updatedAt may have changed)
        setRecs((prev) =>
    ...
    <section aria-labelledby="recommendations-heading">
      ...
      {!loading && !error && visibleRecs.length === 0 && (
        <EmptyState
          ...
          description="Run an analysis to generate crosslink suggestions for this article."
              ...
              <li key={rec.id} className="flex items-start gap-3">
                ...
                <div className="min-w-0 flex-1">
                  <RecommendationCard
                    recommendation={{
                      ...rec,
                      // Disable buttons when a pending bulk op covers this item
