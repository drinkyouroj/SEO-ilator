...
export const authConfig: NextAuthConfig = {
  ...
  callbacks: {
    async signIn() {
      // Allow all sign-ins. Project creation is handled in the session callback
      // because the user record may not exist in the DB yet during OAuth signIn.
      return true;
    },

    async session({ session, user }) {
      // Auto-create a default Project on first session if none exists.
      // This runs here (not in signIn) because the user record is guaranteed
      // to exist in the DB by the time the session callback fires.
